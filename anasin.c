...
void fator(){
    ...
    if(tk.categoria == ID){
        ...
        if(tk.categoria == SN && tk.cod ==  PARENTESIS_ABRE){
            ...
            //Chama a funÃ§Ã£o de expressÃ£o
            //expr();

            //LaÃ§o para checar as ocorrencias de vÃ­rgulas e expr consecutivas
            while(1){
...
void expr()
{
    ...
    expr_simp();
    next_tk = next_token();/*pega um token na frente para olhar se Ã© um op_rel ou fim da exp_simp*/
    if(next_tk.categoria == SN)/*Se for um sinal eu verifico se Ã© um op_rel, se nÃ£o ele sai por vazio*/
    {
        op_rel();
        expr_simp();
